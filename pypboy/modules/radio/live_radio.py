    ...
    def play_song(self):
        ...
        if settings.SOUND_ENABLED:
            if self.files[0].endswith("Silence.ogg"):
                ...
                if hasattr(self, 'last_filename') and self.last_filename and self.last_filename == self.filename:
                    elapsed_time = current_time - self.last_playtime
                    new_start_pos = (self.last_playpos / 1000.0 + elapsed_time) % self.song_lengths[0]
                    print("Resuming song:", self.last_filename, "at", new_start_pos, "seconds after", elapsed_time, "seconds away")
                ...
                print("Playing =", display_name,
                    "length =", str(round(self.song_lengths[0], 2)),
                    "start_pos =", str(round(self.start_pos, 2))
                    )

                settings.FOOTER_RADIO[0] = display_name  # Display only the filename without the extension
                ...
                self.last_playtime = current_time
                self.last_playpos = self.start_pos * 1000  # Save in milliseconds
